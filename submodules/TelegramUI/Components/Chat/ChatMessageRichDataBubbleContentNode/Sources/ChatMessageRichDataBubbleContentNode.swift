...
public class ChatMessageRichDataBubbleContentNode: ChatMessageBubbleContentNode {
            ...
            performAction: { [weak self] text, action in
                ...
                if case .copy = action,
                   ...
                   let adapter = self.textSelectionAdapter {
                    ...
                    if !markdown.isEmpty {
                        item.controllerInteraction.performTextSelectionAction(item.message, true, NSAttributedString(string: markdown), .copy)
                        return
                    }
                }
    ...
    private func activateShowMore() {
        ...
        if attribute.fullInstantPage != nil {
            ...
            item.controllerInteraction.requestMessageUpdate(messageId, false)
            return
        }
